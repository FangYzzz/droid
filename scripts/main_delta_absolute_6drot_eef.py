...
from droid.robot_env import RobotEnv_Franky,RobotEnv
import tqdm
import tyro
...
def rot6d_to_rpy(rot6d):
    ...
    rpy = R.from_matrix(mat).as_euler('xyz',degrees=False)
    return rpy

# def rot6d_to_rpy(x):
#     x = np.asarray(x)
#     M = x.reshape(3, 2)             # [[r11 r12],[r21 r22],[r31 r32]]
#     a = M[:, 0]                     # intended first column
#     b = M[:, 1]                     # intended second column

#     r1 = a / np.linalg.norm(a)
#     b_orth = b - np.dot(r1, b) * r1
#     r2 = b_orth / np.linalg.norm(b_orth)
#     r3 = np.cross(r1, r2)
...
def main(args: Args):
    ...
    while True:
        ...
        for t_step in bar:
            ...
            try:
                ...
                if actions_from_chunk_completed == 0 or actions_from_chunk_completed >= args.open_loop_horizon:
                    ...
                    eef_pose_ = np.concatenate([eef_pose[:3], eef_quat], axis=-1)
                    #######################################
                    if curr_obs["gripper_position"]<0.1:
                    ...
                    request_data = {
                        ...
                        "observation/eef_position": eef_pose_,
                    ...
                    assert pred_action_chunk.shape == (15, 10) # 10,8
                ...
                #----------------------------6d -> rpy----------------------------#
                action_6drot = action[3:9] 
                
                action_rpy = rot6d_to_rpy(action_6drot)
                # print(action_rpy)
                #-------------------------------------------------------------------#
                ...
                # action[:-2] = action[:-2]*2
                # print(action[:3] )
                action_trans = action[:3] + eef_state[:3]
                # print(action[:3] )
                # action[3:6] = action_rpy
                action_ = np.concatenate([action_trans, action_rpy, gripper],axis=-1)
        ...
        if input("Save videos? (enter y or n) ").lower() == "y":
            ...
            save_filename = "video_" + timestamp + "_wirst"
